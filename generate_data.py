...
import logging
logging.disable(logging.CRITICAL)   # disable logging to the terminal by the autolens package
...
def main():
    ...
    """
    params.parse_wrapper(sys.argv[1:], 'data')  # parse the arguments passed through the cli with error handling
    check_dataset_dir()                         # check if the dataset folder exists and is empty
    ...
    grid = al.Grid2D.uniform(shape_native=(params.params['data']['grid-size'], params.params['data']['grid-size']), 
                            pixel_scales=0.05)
    generate_data_files(grid)   # generate the dataset in path/dataset using the specified grid size
    os.system(f'rm ./report.log ./root.log')    # remove log files generated by auto lens
...
def generate_data_files(grid):
    ...
    param_df = pd.DataFrame(data=[], columns=['file', *params.labels_description.keys()])   # a dataframe with the labels
    params.print_script_description('data') # print the desciption of the script functionality to the cli
    ...
    for i in tqdm(range(params.params['data']['generated-number']), desc='Generating .pickle files to ./dataset'):
        sample = params.generate_sample()   # get a random sample of properties using the parameters in params_and_cli.py
        ...
        tracer_result = tracer.image_2d_from_grid(grid) # get the intensity values from the grid with image coordinates
        ...
        param_df.loc[param_df.shape[0]] = df_row    # add the row to the end of the dataframe

    param_df.to_csv(f'{path}/dataset/param_df.csv', index=False)    # save to a csv file without an index column

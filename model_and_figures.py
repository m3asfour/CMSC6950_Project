...
def create_model(input_shape):
    ...
    model.compile(optimizer=optimizers.Adam(params.params['model-figures']['learning-rate']), loss='mse', metrics=['mae'])
    return model
...
def split_data(x, y):
    ...
    test_indexes = np.random.choice(all_indexes, int(x.shape[0]*params.params['model-figures']['test-split']))
    
    all_indexes = np.array(list(set(all_indexes).difference(test_indexes)))
    valid_indexes = np.random.choice(all_indexes, int(all_indexes.size*params.params['model-figures']['valid-split']))
...
def generate_loss_figure(model, history):
    ...
    plt.tight_layout()
    fig.savefig(f'{path}/figures/loss_epochs.{params.params["model-figures"]["figure-format"]}', bbox_inches='tight')
    return ax
...
def generate_sample_figures(x):
    for iter_idx, tracer_img in enumerate(tqdm(x[:params.params['model-figures']['subplots-number']], 
        ...
        ax.axis('off')
        ax.set_title(f'Generated Gravitational Lens Sample #{iter_idx+1}', size=20)
        plt.tight_layout()
